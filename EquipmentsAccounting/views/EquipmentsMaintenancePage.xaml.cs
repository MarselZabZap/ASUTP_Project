...
namespace EquipmentsAccounting.views
{
    ...
    public partial class EquipmentsMaintenancePage : Page
    {
        ...
        private void DataTableFilter()
        {
            ...
            switch (WithoutEndDateCheckBox.IsChecked)
            {
                ...
                case false:
                    ...
                    {
                        EquipmentsMaintenanceDataGrid.DataContext = database.Query(@"SELECT * FROM equipments_service").DefaultView;
                    }
                    else
                    {
                        if (Singleton.START_DATE == null && Singleton.END_DATE == null) // Фильтрация БЕЗ указания диапазона дат
                        {
                            ...
                            {
                                EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(@"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%')",
                                    filterSettings[0], FilterTextBox.Text)).DefaultView;
                            }
                            else
                            {
                                EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(@"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%')",
                                    filterSettings[1], FilterTextBox.Text)).DefaultView;
                            }
                        }
                            ...
                            if (DateTypeFilterToggleButton.IsChecked == false)
                            {
                                if ((bool)NameRadioButton.IsChecked)
                                {
                                    EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                        @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'", filterSettings[0], FilterTextBox.Text, Singleton.START_DATE)).DefaultView;
                                }
                                else
                                {
                                    EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                        @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'", filterSettings[1], FilterTextBox.Text, Singleton.START_DATE)).DefaultView;
                                }
                            }
                            else
                            {
                                if ((bool)NameRadioButton.IsChecked)
                                {
                                    EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                        @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата завершения""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'", filterSettings[0], FilterTextBox.Text, Singleton.START_DATE)).DefaultView;
                                }
                                else
                                {
                                    EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                        @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата завершения""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'", filterSettings[1], FilterTextBox.Text, Singleton.START_DATE)).DefaultView;
                                }
                            }
                        }
                        else // Фильтрация С указанием диапазона дат
                        {
                            ...
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'
                                AND (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) <= '{3}'", filterSettings[0], FilterTextBox.Text, Singleton.START_DATE, Singleton.END_DATE)).DefaultView;
                                }
                                else
                                {
                                    EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                        @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'
                                AND (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) <= '{3}'", filterSettings[1], FilterTextBox.Text, Singleton.START_DATE, Singleton.END_DATE)).DefaultView;
                                }
                            }
                            else
                            {
                            ...
                            (SELECT TO_DATE(to_char(""Дата завершения""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'
                                AND (SELECT TO_DATE(to_char(""Дата завершения""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) <= '{3}'", filterSettings[0], FilterTextBox.Text, Singleton.START_DATE, Singleton.END_DATE)).DefaultView;
                                }
                                else
                                {
                                    EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                        @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'
                                AND (SELECT TO_DATE(to_char(""Дата завершения""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) <= '{3}'", filterSettings[1], FilterTextBox.Text, Singleton.START_DATE, Singleton.END_DATE)).DefaultView;
                                }
                            }
                        }
                    }
                ...
                case true:
                    if (FilterTextBox.Text.Equals("") && Singleton.START_DATE == null && Singleton.END_DATE == null)
                    {
                        EquipmentsMaintenanceDataGrid.DataContext = database.Query(@"SELECT * FROM equipments_service WHERE ""Дата завершения"" is null").DefaultView;
                    }
                    else
                    {
                        if (Singleton.START_DATE == null && Singleton.END_DATE == null) // Фильтрация БЕЗ указания диапазона дат
                        {
                            if ((bool)NameRadioButton.IsChecked)
                            {
                                EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(@"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND ""Дата завершения"" is null",
                                    filterSettings[0], FilterTextBox.Text)).DefaultView;
                            }
                            else
                            {
                        ...
                        else if (Singleton.END_DATE == null) // Фильтрация С указанием даты
                        {
                            if ((bool)NameRadioButton.IsChecked)
                            {
                                EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                    @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}' AND ""Дата завершения"" is null", filterSettings[0], FilterTextBox.Text, Singleton.START_DATE)).DefaultView;
                            }
                            else
                            {
                                EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                    @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}' AND ""Дата завершения"" is null", filterSettings[1], FilterTextBox.Text, Singleton.START_DATE)).DefaultView;
                            }
                        }
                        else // Фильтрация С указанием диапазона дат
                        {
                            if ((bool)NameRadioButton.IsChecked)
                            {
                                EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                    @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'
                                AND (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) <= '{3}' AND ""Дата завершения"" is null", filterSettings[0], FilterTextBox.Text, Singleton.START_DATE, Singleton.END_DATE)).DefaultView;
                            }
                            else
                            {
                                EquipmentsMaintenanceDataGrid.DataContext = database.Query(String.Format(
                                    @"SELECT * FROM equipments_service WHERE lower(""{0}"") LIKE lower('%{1}%') AND
                            (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) >= '{2}'
                                AND (SELECT TO_DATE(to_char(""Дата приёма""::date, 'YYYY-MM-DD'), 'YYYY-MM-DD')) <= '{3}' AND ""Дата завершения"" is null", filterSettings[1], FilterTextBox.Text, Singleton.START_DATE, Singleton.END_DATE)).DefaultView;
                            }
                        }
                    }
                    break;
        ...
        private void RemoveMaintenanceWithChangesButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            if (eqName.Text.Contains("Компьютер"))
            { 
                EditingEquipment window = new EditingEquipment();
                window.ShowDialog();
                DataTableFilter();
            }
        }
